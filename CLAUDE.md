...
6. `component.json` - flat `name`, `registryPackage` `com.lamintra.<segment>`,
   `categories`, `prefix`, `files`, `preview`.
...
**Never use an em-dash or any emoji anywhere in this project.** Source, KDoc,
comments, markdown, commit messages, and anything the CLI prints. Use a plain
hyphen, a comma, a colon, or split the sentence. For status output use plain
words, not a check or cross symbol.

The whole project was purged to pure ASCII on 2026-08-06 (405 em-dashes, 8
emoji, plus en-dashes, ellipses, arrows and math symbols converted to ASCII
equivalents). Keep it that way: a stray non-ASCII character is now a visible
...
## Component naming - DECIDED 2026-08-06: flat slugs, not `category/style`

`lamintra add button`, `add text-field`, `add glass-sheet`. One slug, no path.
`categories` is a metadata array in the manifest, used only for website
grouping - never part of the identifier.

Chosen from live research, not preference: shadcn/ui (`button`), Magic UI
(`@magicui/shimmer-button`), Aceternity UI at **200+ components**
(`@aceternity/magnetic-button`) and RikkaUI (`rikkaui add button card input`)
are all flat, and shadcn's registry spec explicitly rejects nested paths in an
item name. Full reasoning and the competitor table are in `design/TOKENS.md`
under "Naming".

**The scaling worry this answers** - "what happens at 100 buttons?" - has two
different answers and they must not be confused:
- **Variants of one design are a parameter.** `LayerButton(emphasis = ...)`, like
  shadcn's `variant`. These never become registry entries.
- **Genuinely different designs are `<modifier>-<noun>` slugs**, e.g.
  `shimmer-button`. The trailing noun is what makes them groupable without a
  path.

**Kotlin-specific, and ours alone:** the slug becomes a package segment and
hyphens are illegal there. `ComponentManifest.packageSegment` maps `-` -> `_`
(`text-field` -> `text_field`), and manifest load rejects any name that is not
lowercase kebab-case or that would collide with a reserved Kotlin keyword.
Don't "simplify" by using underscores in the command - `add text_field` next to
`npx shadcn add text-field` is exactly the amateurish tell this avoids.

## Current state (update this section as work progresses)

- **DESIGN LANGUAGE REPLACED 2026-08-06 - read `design/TOKENS.md` first.**
  The layer language was **rejected on sight** ("very bad... cheap... even looks
  like material ui"). Replaced by **contrast, space and physics**: solid ink
  fills, capsule actions against rectangular containers, sentence case, and
  **springs instead of `tween`** everywhere. Evidence base is Uber's Base
  system and Apple's HIG/WWDC23, all cited in `TOKENS.md`.
  - **Ink carries action; accent carries state.** Founder chose the accent
    switch on-state, which kills Uber's "one accent per screen" rule - a
    settings screen has ten switches. `LamintraSwitchColors.trackOn` is now the
    only load-bearing colour in the library.
  - **Composables renamed `Layer*` -> `Lamintra*`.** The prefix must not encode
    a design language, because the language changed once and can again. A brand
    prefix also avoids colliding with Material's `Button` in a host app that
    imports both.
  - **The process lesson, worth more than the pixels:** the layer language was
    validated by "can tabs and a slider be derived from it?" - which tests
    *consistency*, not *distinctiveness*. It passed and still looked generic.
    Check any future language against both.

- **Wave 1 - BUILT, COMPILES, awaiting final verdict (2026-08-06).** Five
  components in the registry: `button`, `card`, `text-field`, `list-row`,
  `switch`, all `compose.foundation` only, each with
  `<Component>Colors.dark()/.light()/.auto()`, rest/pressed/disabled/focus, a
  verbatim copy of the tested `Squircle`, a `component.json`, and a `@Preview`
  showing **both** schemes as separate previews rather than a toggle.
  - Compiles: **desktop yes, wasm yes, Android yes** (Android done by staging the
    components into a scratch copy of `jetcompose-kmp-testbed` and running
    `assembleDebug` - this is the only place the five `@Preview` files get
  ...
  - Tests: **23/23 on desktop** (13 interaction tests for the five, 3
    ...
    **mutation-checked**: breaking `enabled` on `LayerSwitch` and drifting one
    ...
    nothing else.
  - Prototypes are **deleted after promotion**, never kept alongside - that
    duplication is the exact drift this project has been bitten by. The harness
    now holds only `DirectionKit.kt` (shared colours, type scale, springs, and
    the two NOT-yet-wave-1 candidates: segmented control and slider),
    `Gallery.kt`, `SettingsScreen.kt` and `Squircle.kt`. All the `CandidateC*`
    files are gone with the layer language.
  - New: `./gradlew :harness:run` opens the harness in a native desktop window
    (`HarnessRoot` - Gallery / Settings screen / Signature tabs).
  - **`VerificationScreen` (the SIGNATURE tab) is still in the old language**,
    because `glass-sheet` and the neon fixtures have not been revisited. The
    jump between tabs is expected, not a bug.
...
- `registry/` - **six components, published as v0.5.2 (2026-08-06)**: the five
  wave-1 base-tier entries above plus `glass-sheet` (signature tier, was
  `bottomsheet/glass`). `neon-button` and `neon-outline-button` were **deleted**,
  not rebuilt: Day-1 fixtures in a language nothing else uses, named after the
  design brief's own stated anti-reference. **Component names are validated at
  manifest load** - lowercase kebab-case only, and rejected if the mapped
  package segment would be a reserved Kotlin keyword. Hyphens are mapped to
  underscores for the package, so kebab-case in a name is now correct rather
  than fatal.

- **Never reference another component by fully-qualified name**, not even in a
  KDoc link. The installer only rewrites a component's OWN registry package, by
  design - another component may not be installed at all. A cross-component FQN
  therefore survives verbatim into a stranger's source and points at a package
  that need not exist there. This shipped in v0.5.0 and took two more tags to
  fully remove, because the fix's own explanatory comment named the package too.
  **It is only findable by grepping the installed output, never by reading the
...
1. Folder-depth complaint from real-dev test (deep
   `ui/components/<slug>/internal/<prefix>/` nesting) - cosmetic, and one level
   shallower since the flat-slug migration dropped the `<category>/<style>`
   split.
2. ~~CLI validation: reject non-package-legal style names at manifest load.~~
   **DONE 2026-08-06** as part of the flat-slug migration - `ComponentManifest`
   validates the slug is lowercase kebab-case and that its mapped package
   segment is not a reserved Kotlin keyword, with tests for both.
